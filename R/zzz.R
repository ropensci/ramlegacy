.onAttach <- function(libname, pkgname) {
  ...
  if (is.null(local_vers)) {
    not_completed(paste(
      "No version of the database has yet been downloaded.",
      "Use function download_ramlegacy() to download a version now."
    ))

    # if multiple versions found locally then prompt the user to select one
    ...
    notify(paste(
      "Multiple versions found including the latest one:",
      latest_version
    ))
